...
export const CardListModeSelector: React.FC<TCardListModeSelectorProps> = ({sides}) => {
	...
	return <div className={'list-mode-selector'}>
		...
		{currentStyle === 'table' && (cardbox?.cards?.length || 0) > 0 && <div className={'table-mode-selector'}>
		...
		{currentStyle === 'cards' && (cardbox?.cards?.length || 0) > 0 && <div className={'card-side-selector'}>
			<div className={'pure-button-group'}>
				{sides?.map((side, idx) => {
					return <Button
		...
		{currentStyle === 'table' && (cardbox?.cards?.length || 0) > 0 && <div className={'table-wide-selector'}>
			<div className={'pure-button-group'}>
				<Button
					icon={<TbViewportWide/>}
